...
public class VersionContext implements VersionInterface {
  ...
  private static final long LIMIT_PER_RUN = 5000;
  ...
  public void dumpINodeInDetail(String path, HttpServletResponse resp) throws IOException {
    ...
    try {
      ...
      nodeDetails.put("aclsCount", ((aclFeature == null) ? "NONE" : aclFeature.getEntries().size()));
      if (node.isFile()) {
        ...
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, namesystem, LIMIT_PER_RUN);
          return x.computeContentSummary(cscc).getCounts().get(Content.DISKSPACE);
        };
      case "dirSubTreeNumFiles":
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, namesystem, LIMIT_PER_RUN);
          return x.computeContentSummary(cscc).getCounts().get(Content.FILE);
        };
      case "dirSubTreeNumDirs":
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, namesystem, LIMIT_PER_RUN);
          return x.computeContentSummary(cscc).getCounts().get(Content.DIRECTORY);
        };
      case "storageType":
